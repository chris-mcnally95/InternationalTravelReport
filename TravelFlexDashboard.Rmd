...
totalnonred <- nrow(travellers.status %>% 
                     filter(Status == "Non-Red",
...
fortnonred <- nrow(FortTravellers %>% 
                    filter(Status == "Non-Red"))
...
Please note that in some cases a country status might not be assigned and will result in the omission of data. Furthermore, as of the 4th of October 2021, the Red, Amber, Green status of countries has been deprecated and replaced by Red and Non-Red status. Total cases refers to all cases since records began in March 2020. Two weeks refers to all cases from Monday 00:00 on `r format(floor_date(today()-13, "week")+1, "%d-%m-%Y")` to the most recent Sunday 23:59.
...
### 
```{r}
valueBox(value = totalnonred,
         caption = paste("Total Reported Cases from Non-Red Countries"),
         icon = "ion-plane", 
         color ="forestgreen")
```
...
### 
```{r}
valueBox(value = fortnonred,
         caption = paste("Reported Cases from Non-Red Countries Within Last Two Weeks"),
         icon = "ion-plane", 
         color = "forestgreen")
```
